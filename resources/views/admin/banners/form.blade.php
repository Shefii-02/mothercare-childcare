    ...
    <div
        ...
        <nav>
            <ol class="flex flex-wrap items-center gap-2">
                ...
                <li>
                    <a class="flex items-center gap-3 font-medium" href="{{ route('admin.banners.index') }}">
                      ...
                      <span class="hover:text-primary">Banners</span>
                    </a>
                  </li>
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div class="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                <h2 class="text-title-sm2 font-bold text-black dark:text-black">
                    {{ isset($banner) ? 'Edit Banner' : 'Create a Banner' }}
                </h2>
            </div>
            ...
            <form enctype="multipart/form-data"
                ...
                <div class="p-6.5 ">
                    ...
                    <div class="mb-5 w-full">
                        <label class="block text-sm font-medium text-black dark:text-dark">Desktop Image</label>
                        <div x-data="imageHandler('{{ isset($banner) && $banner->desktop ? asset('images/' . $banner->desktop) : '' }}')" class="flex gap-15 mt-2">
                            <!-- Image Upload Component -->
                            <template x-if="imageUrl">
                                <div class="relative">
                                    <img :src="imageUrl" class=" h-40 object-cover rounded-lg" />
                                    <button type="button"
                                        class="absolute top-0 right-0 bg-danger text-white rounded-full p-1"
                                        @click="removeImage">
                                        &times;
                                    </button>
                                    @if (isset($banner->desktop))
                                        <input type="hidden" value="1" name="exist_desktop_image">
                                    @endif
                                </div>
                            </template>
                            <label for="desktop_image"
                                ...
                                <span class="small">Recommended resolution 1920*700 with Max size 100kb</span>
                                <small></small>
                                <input type="file" id="desktop_image" name="desktop_image" class="hidden"
                    ...
                    <!-- Mobile Image -->
                    <div class="mb-5 w-full">
                        <label class="block text-sm font-medium text-black dark:text-dark">Mobile Image</label>
                        <div x-data="imageHandler('{{ isset($banner) && $banner->mobile ? asset('images/' . $banner->mobile) : '' }}')" class="flex gap-15 mt-2">
                            <!-- Image Upload Component -->
                            <template x-if="imageUrl">
                                <div class="relative">
                                    <img :src="imageUrl" class=" h-40 object-cover rounded-lg" />
                                    ...
                                    @if (isset($banner->mobile))
                                        <input type="hidden" value="1" name="exist_mobile_image">
                                    @endif
                                </div>
                            </template>
                            ...
                            <label for="mobile_image"
                                class="block mt-3 cursor-pointer bg-gray-50 border-2 border-dashed border-gray-300 p-4 rounded-lg text-center text-sm text-gray-500 hover:bg-gray-100 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-400">
                                <span>Click to upload Mobile Image</span><br>
                                <span class="small">Recommended size 760*920 with Max size 100kb</span>
                                <input type="file" id="mobile_image" name="mobile_image" class="hidden"
                                    @change="handleFileUpload">
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="mb-5 w-1/4">
                        ...
                        <select class="form-control mt-2" name="display_order" required>
                            @for ($i = 1; $i < 10; $i++)
                                <option {{ isset($banner) && $banner->display_order == $i ? 'selected' : '' }}
                                    value="{{ $i }}">{{ $i }}</option>
                            @endfor
                        </select>
                    </div>
                    ...
                    <!-- Status Toggle -->
                    <div class="mb-5">
                        <label class="block text-sm font-medium text-black dark:text-dark">Status</label>
                        <div class="form-group mb-3 col-md-4">
                            <label class="form-check form-switch">
                                <input class="form-check-input" name="status" type="checkbox" value="1" id="status"
                                    {{ old('status', isset($banner) && $banner->status ? 'checked' : '') }}>
